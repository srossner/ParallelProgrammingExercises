...
void fill(tree *node, int level)
{
  ...
  node->right = NULL;
  node->id = g_node_id;
  sprintf(node->name, "%s %s %d",position[g_node_id % 39],company[g_node_id % 26], g_node_id/39);
  
  //computing data for each node
  node->data = (123*(g_node_id + node->name[1] * g_node_id / 39)) % 9113; 
  node->work_hours = 0;
  g_node_id++;
